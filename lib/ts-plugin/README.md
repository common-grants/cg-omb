# @common-grants/cg-federal

A [CommonGrants](https://commongrants.org) TypeScript SDK plugin for the
**federal grant data standards**, centered on the **Notice of Funding
Opportunity (NOFO) Information Collection (IC)**.

> **Status: early scaffolding.** The published schema, custom fields, and
> transforms are under active development.

## What this package provides (planned)

- A **NOFO IC JSON Schema** composed from the federal Standard Data Elements
  (SDEs), preserving `$ref`s into the upstream SDE definitions.
...
- **`toCommon` / `fromCommon`** transforms mapping the NOFO IC ↔ CommonGrants
  core, driven by the federal ↔ CommonGrants data-mapping crosswalk.

## Layers

| Layer                               | Ownership    | Location                            |
| ----------------------------------- | ------------ | ----------------------------------- |
| SDE JSON (upstream truth)           | Federal WG   | `schemas/sde/*.schema.json`         |
