/**
 * End-to-end example: translate an opportunity between Grants.gov and the OMB
 * NOFO IC, using CommonGrants as the interchange format.
 *
 *   Grants.gov  --cg-grants-gov.toCommon-->  CommonGrants  --cg-omb.fromCommon-->  OMB NOFO IC
 *
...
const omb = cgOmb.schemas.Opportunity;
...
function main() {
  ...
  const toOmb = omb.fromCommon(common);
  heading("3. → OMB NOFO IC");
  console.log(JSON.stringify(toOmb.result, null, 2));
  reportErrors("cg-omb.fromCommon", toOmb.errors);
  ...
  reportErrors("cg-omb.toCommon", backToCommon.errors);
